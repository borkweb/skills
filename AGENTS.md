...
- **layman** — Restates something in plain, succinct, jargon-free English. User-invoked only — never auto-triggers on dense output. Bare `/layman` restates the assistant's previous message; also accepts inline text or a file path. Returns only the plain version (no preamble, no glossary), always shorter than the source, with code, warnings, and decisions preserved; big numbers are abbreviated (`200k`) but never rounded. Opt-in sticky mode (`/layman on`) keeps every reply plain until stopped. Never edits files.
- **offload** — Architect-mode builder orchestration. The builder harness (codex/claude/opencode/pi/grok or a custom command) comes from `~/.borkweb-skills/config.json` — an ordered failover chain resolved by `harness.mjs` with quota-aware demotion via quota-axi; the skill prompts for and writes the config when missing. Reads session-keyed handoffs from `~/.agent-handoffs/offload/`, arbitrates builder disagreements, judges gate results against frozen criteria, specs the next one-PR slice, and dispatches a builder block into a new herdr tab/tmux window (falls back to Terminal, then headless). `handoff.mjs` also carries the architect's slice **ledger** and the `board` roll-up that `complete` uses to track concurrent builders, plus `section` subcommands for editing a handoff without hand-splicing its markdown. `pane.mjs` enforces the launch invariant: a pane recorded on the ledger must carry dispatch.sh's signature (a herdr tab labeled `builder` holding one pane), so a builder hand-started with `herdr agent start --split` is refused on write and flagged `UNMANAGED PANE` on the board instead of reading as dispatched forever. The architect never writes implementation code; the human is the final judge.
...
Skill entrypoints: commit, handoff, humanize, offload, prototype, session-budget, layman, plan-session, plan-deep-review, plan-eng-review, plan-design-review, plan-devex-review, review, review-security, qa, qa-only.
