...
## 4. Resolve authorization once

Carry the user's existing dispatch, commit, push and merge instructions into the ledger. Do not repeat settled questions. If dispatch would relax permissions beyond existing authorization, identify the actual configured harness flags and obtain that authorization before launch; otherwise proceed. Do not reinterpret merge-ready as merge permission.
...
design fork — use `council` when the stakes justify multiple perspectives; resolve routine choices from the frozen scope and available evidence.
The builder block already instructs the builder to report material unresolved ambiguity before dependent coding (see the offload builder block).
...
with the authorized commit/push steps completed (or an explicit no-push record). Run `board` and confirm that before anything
...
3. **Merge — ONLY if the goal explicitly authorized it (step 1).** Then use the existing merge authorization without asking again; integrate via the repo's
   standard path (the builder completes the authorized delivery steps for each slice, so this is the
